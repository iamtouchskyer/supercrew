    ...
    <div className="rb-page" style={{ height: '100%', overflowY: 'auto', background: 'hsl(var(--_background))' }}>
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '28px 32px 48px' }}>
        ...
        >
          ...
          {/* Tags */}
          {task.tags.length > 0 && (
            <PropCell label={t('task.props.tags')} full>
          ...
          {/* PR */}
          {task.pr_url && (
            <PropCell label={t('task.props.pr')} full>
              <a
                href={task.pr_url}
                target="_blank"
                rel="noreferrer"
          ...
          {task.blocked_by.length > 0 && (
            <PropCell label={t('task.props.blockedBy')} full>
          ...
          {task.blocks.length > 0 && (
            <PropCell label={t('task.props.blocks')} full>
...
function BodyEditor({ body, onSave }: { body: string; onSave: (body: string) => void }) {
  ...
  if (!editing) {
    return (
      ...
      >
        {body || <span style={{ fontStyle: 'italic', opacity: 0.6 }}>{t('task.addDescription')}</span>}
      </div>
    )
  }
